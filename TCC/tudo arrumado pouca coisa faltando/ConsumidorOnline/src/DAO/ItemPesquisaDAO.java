...
//adiciona um novo item de pesquisa
    ...
    public int retornaNumeroDePesquisas(int idCliente) {
        PreparedStatement stmt = null;
        ResultSet rs = null;
    ...
    public void adiciona(int id, String nome, float preco, boolean status, boolean encontrou) {
        String sql = "insert into itempesquisa (id_cliente, nome_pesquisa, preco, status, encontrou) values (?,?,?,?,?)";
...
// recolhe id e preco do item de pesquisa com base que vc ja tenha o nome da mesma e sua id
    ...
    public void updateEncontrado(boolean encontrou, int id) {
        String sql = "update itempesquisa\n"
                + "set encontrou = ?\n"
                + "where itempesquisa.id_pesquisa = ?";
    ...
    public List<ItemPesquisa> listaItensPesquisados(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ip.setStatus(rs.getBoolean("status"));
                ip.setPreco(rs.getString("preco"));
                ip.setEncontrou(rs.getBoolean("encontrou"));
